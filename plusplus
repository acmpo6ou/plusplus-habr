...
plusplus – это маленькая библиотека которая добавляет операторы унарного инкремента и декремента в python.
...
Тем не менее в python конструкция ++x синтаксически корректна, так как он парсит ее как две последующие операции +x, где + это унарный плюс (то же самое с --x и унарным минусом).
...
Тем не менее существуют ситуации, когда инкременты могут упростить код (здесь можете посмотреть примеры из стандартной библиотеки python).
Также инкременты похожи на выражения присваивания (те которые используют оператор := появившийся в PEP572).

Как все устроено

Патчинг байт-кода

Python компилирует весь исходный код в низкоуровневый байт-код который исполняется в виртуальной машине питона. Каждая инструкция байт-кода берет несколько элементов из стека, делает что-нибудь с ними и возвращает результат назад в стек.
Выражение ++x компилируется в две последующие инструкции UNARY_POSITIVE которые не сохраняют никакого промежуточного результата (то же самое и с --x и двумя инструкциями UNARY_NEGATIVE).
...
Дизассемблим и собираем байт-код назад мы с помощью вот этой библиотеки.

Инкременты для целого пакета

Система импорта в Python позволяет загружать модули не только из файлов, но и с других обоснованных мест (например есть модуль который позволяет импортировать код из ответов на StackOverflow). Все что надо сделать это предоставить контент модуля, включая его байт-код.
Таким образом мы можем написать загрузчик, который импортирует модуль и патчит его байт-код. Для этого надо создать новый MetaPathFinder и установить его в sys.meta_path.
